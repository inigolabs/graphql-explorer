...
const Form: React.ForwardRefRenderFunction<FormRef, FormProps> = (props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    setValue: (value) => {
      Object.entries(childrenRefs.current).forEach(([name, childRef], index) => {
  ...
  const onChildChange = (childName: string, childValue: unknown, type?: HTMLInputElement['type']) => {
    const newValue = {
      ...value,
  ...
  const transformChildren = (children: FormProps['children']) => {
    ...
    const result = React.Children.map(arr, (child, i) => {
      if (React.isValidElement(child) && (child.props as any)?.name) {
        ...
        if (['TextInput', 'Select', 'Checkbox'].includes((nextChild as any)?.type?.displayName)) {
          isLastScalar = false;
        }

        return React.cloneElement(child, {
          ref,
          className: classNames((child.props as any).className, isLastScalar && '__last-scalar'),
          ...
          shouldRenderErrorMessages:
            props.shouldRenderErrorMessages ?? (child.props as any).shouldRenderErrorMessages ?? true,
        } as any);
      }
      return child;
    });
  ...
  return (
    <div className={classNames(styles.container, props.title && styles.hasTitle)} style={props.containerStyle}>
