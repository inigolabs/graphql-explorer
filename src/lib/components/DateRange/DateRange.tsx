...
function DateRange(props: IDateRangeProps) {
  ...
  const setDateFromInput = (value: string, index: 0 | 1) => {
    if (value) {
      if (index === 0) {
        const newValue = moment(value);

        if (newValue.isValid()) {
          setDateSelected([newValue, dateSelected[1]]);
        ...
        if (newValue.isValid()) {
          setDateSelected([dateSelected[0], newValue]);
          ...
          setDateSelected([dateSelected[0], TODAY.clone()]);
