      ...
      <table className={styles.Table}>
        ...
        <tbody>
          {props.lines.map((line) => {
            const value: number =
              props.point![
                line.dataKey as keyof LineChartProps["data"]["points"][0]
              ];
  ...
  useEffect(() => {
    setTimeout(() => {
      if (containerRef.current) {
        const tooltipWrapper = containerRef.current.querySelector(
    ...
    <div className={styles.Container} ref={containerRef}>
        ...
        {!!(props.title || props.legend) && (
          <div className={styles.Header}>
            ...
            {!!props.legend && (
              <div className={styles.Legend}>
                <LineChartLegend
                  onSelect={(selected) => setVisible(selected)}
                  lines={props.lines}
                  data={props.data}
                />
              </div>
              ...
              {[...props.lines].reverse().map((line) => (
                <Recharts.Line
                  ...
                  activeDot={(dotProps: any) => (
                    <LineChartDot color={line.color} {...dotProps} />
                  )}
                />
              ))}
            </Recharts.LineChart>
      ...
      {!props.disableXAxis && (
        <div
          ...
          style={props.enableBorder ? { marginTop: 0 } : {}}
        >
