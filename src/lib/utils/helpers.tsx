        ...
        .map(
          ([fieldName, fieldValue]: [
            ...
            if (coverage) {
              ...
              } as unknown as ISchemaPropsItemProperty;
            }
...
const getTimeFilterByName = (timeFilterName: string) => {
  return timeFilters.find((item) => item.name === (timeFilterName as string));
};
...
function schemaDocEnumToStr(enumKey: string, enumInfo: any) {
  ...
  result += Object.entries(enumInfo.Values)
    .map(([key, value]: [any, any]) => `\t${value.Name}`)
    .join("\n");
